...
import { Effect, Exit, FileSystem, Layer, Option, Ref } from 'effect';
...
describe('§9.6 the index is read once, not once per query', () => {
  it.scopedLive('parses the artifact at layer start and never again', () =>
    Effect.gen(function* () {
      // A byte source that counts its own reads. The index bytes are the same
      // every time, so no result assertion could tell one read from twenty —
      // the count is the only observable that distinguishes a scoped resource
      // from a per-query parse.
      ...
      // `SearchService.Live` directly, not `goldenSearchLayer`: the fixture
      // builder supplies its own `VectorIndexBytes` from the `index` argument,
      // which would shadow the counting source and make this test read zero.
      const search = SearchService.Live.pipe(
        Layer.provide(goldenSearchSources),
        Layer.provide(counting),
...
describe('§6.5 the filesystem reader degrades on faults and not on defects', () => {
  ...
  it.scopedLive('reports a missing file as an absent index', () =>
    Effect.gen(function* () {
      ...
      const bytes = yield* Effect.flatMap(VectorIndexBytes, (source) => source.read).pipe(
        Effect.provide(
  ...
  it.scopedLive('lets a defect in the stat propagate', () =>
    Effect.gen(function* () {
      ...
      const outcome = yield* Effect.exit(
        Effect.flatMap(VectorIndexBytes, (source) => source.read).pipe(
          Effect.provide(
