...
const captureWarnings = <A, E, R>(
  ...
  Effect.gen(function* () {
    ...
    yield* effect.pipe(Effect.provide(Logger.layer([recorder])));
    return captured;
  });
...
describe('section composer', () => {
  ...
  test('emits the §6.1 lineup in order with the specified caps', () =>
    Effect.gen(function* () {
      ...
      }).pipe(Effect.provide(live(file, [catalogTopic('naves.sanctuary', 12)])));
    }));

  test('marks key verses default-open and every other section collapsed', () =>
    Effect.gen(function* () {
      const file = yield* artifact([
        { slug: 'sanctuary', title: 'The Sanctuary', thesis: THESIS, catalogId: 'naves.sanctuary' },
      ]);
      yield* Effect.gen(function* () {
        const wiki = yield* WikiService;
        const page = yield* wiki.topic(topicSlug('sanctuary'));
      ...
      }).pipe(Effect.provide(live(file, [catalogTopic('naves.sanctuary', 3)])));
    }));
  ...
  test('orders related topics authored-first, then backlinks, in edge position', () =>
    Effect.gen(function* () {
      ...
      yield* Effect.gen(function* () {
        ...
        expect(related.items.map((item) => item.kind)).toEqual([
          'authored',
          'authored',
          'backlink',
        ]);
  ...
  test('keeps the lineup whole when the corpus has nothing to say', () =>
    Effect.gen(function* () {
      const file = yield* artifact([{ slug: 'sanctuary', title: 'The Sanctuary', thesis: THESIS }]);
  ...
  test('composes a page with no artifact at all, from the catalog alone', () =>
    Effect.gen(function* () {
      ...
      yield* Effect.gen(function* () {
        ...
        expect(page.sections[4]?.items.length).toBe(
          Option.getOrThrow(SECTION_CAPS.crossReferences),
        );
  ...
  test('composes key verses for a topic whose references are all ranges', () =>
    Effect.gen(function* () {
      ...
      }).pipe(Effect.provide(live(file, [rangesOnly])));
    }));
  ...
  test('keeps catalog position across a mixed list of verses and ranges', () =>
    Effect.gen(function* () {
      ...
      }).pipe(Effect.provide(live(file, [mixed])));
    }));
  ...
  test('reads text across a book boundary rather than rendering the range textless', () =>
    Effect.gen(function* () {
      ...
      }).pipe(Effect.provide(live(file, [crossBook])));
    }));
  ...
  test('warns with the token and the slug when the catalog stores an unusable range', () =>
    Effect.gen(function* () {
      ...
      const warnings = yield* captureWarnings(
        ...
        }).pipe(Effect.provide(live(file, [rejected]))),
      );
        ...
        Effect.gen(function* () {
          ...
          Effect.provide(
            WikiSectionSources.NotWiredOnCorpusAbsence(
              // The real shape of an absent corpus on this stack: `bun:sqlite`
              // throws from its constructor, so a missing file arrives as a
              // died `SQLITE_CANTOPEN` and never as a typed failure.
              WikiSectionSources.Live.pipe(
                Layer.provide(TopicService.Test([])),
                Layer.provide(BibleDatabase.layerTest({})),
  ...
  test('gives a catalog page the live backlinks pointing at it', () =>
    Effect.gen(function* () {
      ...
      }).pipe(Effect.provide(live(file, [catalogTopic('naves.grace', 0)])));
    }));
