export const meta = {
  name: 'handbook-factory',
  description:
    'The canonical handbook factory: read a spec file → gather witnesses (Bible + EGW/pioneer) → dual-agent section creation → cross-agent section critique → synthesizer → dual-agent section verifier (auto-fix loop, capped) → aggregator → dual-agent final review (auto-fix loop, capped). Builds ANY handbook from a JS spec. Pass {specPath} as args.',
  whenToUse:
    'Generate any Bible handbook / Sabbath School study from a spec.js. The spec supplies title, thesis, sections[], bibleOnly, template path. Pass args={specPath:"/abs/path/to/x.spec.js"}.',
  phases: [
    { title: 'Spec', detail: 'read + validate the handbook spec file' },
    {
      title: 'Witnesses',
      detail:
        'per section: gather the cloud of witnesses — Bible primary, EGW/pioneer secondary, all verified',
    },
    {
      title: 'Draft',
      detail:
        'dual-agent section creation: two writers draft each section independently from the witnesses',
    },
    {
      title: 'Critique',
      detail:
        'cross-agent critique: each draft adversarially reviewed against the other + the witnesses',
    },
    {
      title: 'Synthesize',
      detail: 'opus synthesizer fuses the two drafts + critique into one section',
    },
    {
      title: 'Verify-Section',
      detail:
        'dual-agent verifier per section (citations + coherence); auto-fix loop capped at 2 rounds',
    },
    {
      title: 'Aggregate',
      detail:
    ...
    },
    {
      title: 'Final-Review',
      detail:
        'dual-agent whole-document review + reconciler; fixes applied at section source, then re-assembled; capped at 2 rounds',
    },
  ],
};
...
if (!SPEC_PATH)
  throw new Error('handbook-factory: pass args={specPath:"/abs/path/to/handbook.spec.js"}');
...
//   witnesses → dual draft → cross critique → synth → verify+fix loop → save
// ===========================================================================
phase('Witnesses');
...
const built = await pipeline(
  ...
  // Stage 1: gather the cloud of witnesses (Bible primary, EGW/pioneer verified)
  (s) =>
    agent(
...
Be exhaustive on the Bible spine (8-16 verses). ${BIBLE_ONLY ? 'At most 0-2 pioneer refs, all historical/confirmation.' : '6-12 verified witnesses.'} Your final message IS the structured witnesses.`,
  ...
  // Stage 2: dual-agent section creation — two independent writers from the witnesses
  (prev) => {
    if (!prev) return null;
    const { s, w } = prev;
...
${JSON.stringify(w)}`;
  ...
  // Stage 3: cross-agent critique — adversarially compare the two drafts
  (prev) => {
    if (!prev) return null;
...
Return the structured critique.`,
  ...
  // Stage 4-6: opus synthesize → dual-verify + auto-fix loop → persist
  async (prev, s, i) => {
    if (!prev) return null;
    const { w, draftA, draftB, critique } = prev;

    let md = await agent(
      `You are the OPUS SYNTHESIZER for handbook section ${i + 1}, "${s.title}", Part ${s.part}.
...
// ===========================================================================
// Stitching, renumbering, the TOC, and the Symbol Dictionary are pure text ops;
// an LLM doing them by hand is slow and leaks narration / drops bodies. The CLI
// reads a handbook.json manifest + the per-section files and assembles in one
// call. One agent writes the manifest and runs the command (the sandbox has no
...
const manifest = {
  ...
  createdAt: spec.createdAt || '2026-06-20T12:00:00Z',
  thesis: spec.thesis,
  method: spec.method,
...
// ===========================================================================
// Phase 8 — dual-agent final review + reconciler + auto-fix loop (capped)
// ===========================================================================
// The review reads the assembled file from disk. Fixes are applied to the
// per-section files (the source of truth), then the CLI re-assembles — so a fix
// can never corrupt the stitching, and the appendix/TOC always stay in sync.
phase('Final-Review');

let finalResidual = [];
for (let round = 0; round < FINAL_FIX_ROUNDS; round++) {
  // Read the freshly-assembled handbook from disk for review.
  const handbook = await agent(
    ...
    { label: `read:r${round}`, phase: 'Final-Review', agentType: 'general-purpose' },
  );
  ...
  // reconciler cross-checks the two reviews (re-verifies single-source flags)
...
Return the structured verdict (only CONFIRMED fails, ranked).`,
  ...
  // Apply fixes to the SECTION FILES at source (frontmatter/TOC/appendix are
  // regenerated by the CLI, so never hand-edit those — only the section bodies).
  await agent(
...
${JSON.stringify(finalResidual, null, 2)}`,
  ...
  // Re-assemble from the corrected section files.
