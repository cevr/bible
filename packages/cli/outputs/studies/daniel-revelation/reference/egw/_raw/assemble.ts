...
for (const [study, sections] of byStudy) {
  ...
  const lines: string[] = [];
  lines.push(`# EGW — GC + PK References for Study ${study}`);
  lines.push('');
  lines.push(`**Study:** ${title}`);
  lines.push('');
  lines.push(
    '> Mined from Ellen White, *The Great Controversy* (GC) and *Prophets and Kings* (PK) via local FTS index. Per series rule: **no EGW quotes in the studies themselves** — use these to find the right Bible texts, then let Scripture speak.',
  );
  lines.push('');
