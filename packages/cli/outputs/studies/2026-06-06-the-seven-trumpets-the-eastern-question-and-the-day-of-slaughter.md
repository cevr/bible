...
_Method: Bible as it reads; Scripture its own expositor (Rule 5); gather every passage before the thesis (Rule 4); trace each figure till explained (Rule 12); test history against prophecy word for word (Rule 13). Pioneers — Litch, Smith, Andrews — glossed to their refcode (DAR, SSTR, PREX, MWV, TMR; EGW by GC/EW/PP/TM…), so every source is one lookup away._
...
- _GC 389.2._ Second message ("Babylon is fallen") — powered by the **first message's rejection**; preached only after the churches refused the hour-of-judgment cry ("first preached in the summer of 1844... most widely proclaimed and most generally rejected").
