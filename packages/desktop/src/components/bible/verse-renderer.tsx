...
// @bible/core/bible-rendering. The desktop drawer is read-only and has no
// margin-note popover UI yet — anchors still render as muted superscript
// letters so the position is visible, with a `title` carrying the index for
// future popover wiring.
//
...
const Segment: Component<{ readonly segment: TextSegment }> = (props) => (
  <Switch>
    ...
    <Match when={props.segment.type === 'margin' ? props.segment : null}>
      {(s) => (
        <sup
          ...
          title={`Margin note ${String(s().noteIndex + 1)}`}
        >
...
export const VerseRenderer: Component<VerseRendererProps> = (props) => {
  const segments = (): readonly TextSegment[] =>
    segmentVerseText(props.text, [], props.searchQuery);
  return <For each={segments()}>{(seg) => <Segment segment={seg} />}</For>;
};
