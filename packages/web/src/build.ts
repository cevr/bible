...
for (const meta of STUDIES) {
  ...
  built.push({ slug: meta.slug, words, sections: toc.length });
  log(
    `  ✓ /studies/${meta.slug}/  (${toc.length} sections, ${Math.round(words / 1000)}k words)`,
  );
}
...
await Bun.write(join(dist, 'index.html'), indexPage({ studies: studiesWithStats, comparisons: COMPARISONS }));
await Bun.write(
  join(dist, '404.html'),
