...
import { Service } from './reference-links.js';
...
const linkReferences = (html: string, panels = PANELS): string =>
  Effect.runSync(
    Effect.map(Service, (references) => references.link(html)).pipe(
      Effect.provide(Service.Test(panels)),
    ),
  );
...
describe('linkReferences', () => {
  ...
  test('keeps unresolved publication-like references as text', () => {
    expect(linkReferences('<p>UNKNOWN 12.3</p>')).toBe('<p>UNKNOWN 12.3</p>');
  });
  ...
  test('does not create nested links or touch code and tag attributes', () => {
    ...
    expect(linkReferences(html)).toBe(html);
  });
});
