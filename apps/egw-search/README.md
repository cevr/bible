...
`ActorHost`/`HttpServer` fixture on port `3187`, and runs Playwright against
that server. The fixture uses deterministic search responses. It does not
...
The fixture checks the HTTP batch transport, URL history, row identity,
typed query failures, recovery, request cancellation, and query resource
release. It does not prove production corpus retrieval or browser behavior in
